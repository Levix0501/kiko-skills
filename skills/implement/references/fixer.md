...
Require `Role: fixer`, `Action: execute|evidence-recovery`, `Mode: intermediate|final`, `Branch`, absolute Spec, Scope and Output paths, Evidence sources, and Finding sources.
...
- fix each supplied root problem without expanding the current contract;
- preserve the intermediate stable landing, or in final mode preserve the whole integrated product contract;
- follow repository instructions and, before DONE, run affected tests, integration checks, Scope Verify, and acceptance verification;
